...
const employeeDataArray = JSON.parse(employeeData);

// task 1

const newEmployeeData = data => data.map(obj => ({ name: obj.name, position: obj.hr.position, salary: obj.hr.salary }))
console.log(newEmployeeData(employeeDataArray));
...
for (const index in employeeDataArray) {
    cityArr.add(employeeDataArray[index].contact[0]);
}

const numberEmployeesByCity = Object.assign(...Array.from(cityArr, v => ({ [v]: '' })));

for (let index in employeeDataArray) {
    if (cityArr.has(employeeDataArray[index].contact[0])) {
        numberEmployeesByCity[employeeDataArray[index].contact[0]]++;
    }
}
...
setTimeout(() => {
    ...
    const employeeNames = employeeDataArray.map((key) => key.name);
    console.log('Մեր բոլոր աշխատակիցները։', employeeNames.join(', '))

    const removeEmployee = employeeNames[Math.floor(Math.random() * (employeeNames.length))];
    console.log('Ներող ապեր քեզ դուրս ենք հանում:', removeEmployee);

    const newEmployees = employeeNames.filter((name) => name !== removeEmployee);
    console.log('Նոր աշխատակիցների ցուցակ։ ', newEmployees.join(', '));
